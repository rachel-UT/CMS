...
class AboutFrame(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        ttk.Label(self, text = "About Author: Rachel Zhang").pack()
        ttk.Label(self, text = "Email: hzhang@ut.edu").pack()
...
class CreateFrame(ttk.Frame):
    ...
    def create_customer_data(self):
        ...
        require_value = [self.fname.get(), self.lname.get(),
...
class UpdateFrame(ttk.Frame):
    ...
    def show_customer_data(self):
        ...
        result = db.query_table_with_condition("customers", "*", conditions)
        print(result)
        if len(result) == 0:
    ...
    def update_customer_data(self):
        ...
        if self.email.get()== "":
            tk.messagebox.showerror("Error",
                                    "This email address doesn't exit.")
        else:
            flag = db.update_customer_data(self.fname.get(), self.lname.get(), self.gender.get(),
                                           self.dob.get(), self.mobile.get(), self.address.get(),
                                           self.city.get(), self.state.get(), self.zipcode.get(),
                                           self.email.get())
            if flag == True:
...
class PaymentFrame(ttk.Frame):
    ...
    def create_page(self):
        ...
        ttk.Label(self.payment, text="Payment Amount: ", font=("Calibri 12")).grid(row=1, column=0, pady=10,
                                                                                   sticky=tk.W)
...
class SearchFrame(ttk.Frame):
    ...
    def show_payment_data(self):
        ...
        if len(field_value) == 0:
            result = db.query_table("payments", "*")
